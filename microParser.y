...
unordered_map<char *,char *> function_return_type;
int param_count = 0;
stack <int> arg_count;
...
%type <var> vartype any_type VOID
...
func_decl: FUNCTION any_type id
                        {
                                ...
                                code->instructions[code->num_entries]=(char *)malloc(200*sizeof(char));
                                sprintf(code->instructions[code->num_entries],"link ");
                                code->num_entries++;
...
'(' param_decl_list ')' _BEGIN func_body END
                {
                        for(int i=0;i<code->num_entries;i++)
                        {
                                if(!strcmp("link ",code->instructions[i]))
                                {
                                        ...
                                        for(int j=0;j<=table_index;j++)
                                        {
                                                if(current_scope != NULL)
                                                {
                                                        if(!strcmp(current_scope,table_list[j].name))
                                                        {
                                                                ...
                                                                for(int k=j;k<=table_index;k++)
                                                                {
                                                                        var_count+=table_list[j].num_entries;
                                                                }
                                                                sprintf(code->instructions[i],"link %d",max(5,var_count));
                                                        }
                                                }
                                        }
                                        break;
...
call_expr: id{arg_count.push(0);} '(' expr_list ')'
        {
                ...
                for(auto x:function_return_type)
                {
                        if(!strcmp($1,x.first))
                        {
                                $$->vartype = strdup(x.second);
                        }
                }
                if($4 != NULL)
                {
        ...
        ;
expr_list: expr{free_register($1->register_num);} expr_list_tail 
        {
                arg_count.top()++;
                if($3 == NULL)
                        $$ = $1;
                else
                {
                        ...
                        for(int i=0;i<$3->num_entries;i++)
                        {
                                $$->instructions[i]=$3->instructions[i];
                        }
        ...
        | {$$ = NULL;}
        ;
expr_list_tail: ',' expr {arg_count.top()++; free_register($2->register_num);} expr_list_tail
        {
                if($4 == NULL)
                        $$ = $2;
                else
